...
const MERGE_LANGS = new Set<SupportedLanguage>(["es", "fr", "en", "ja"]);

const NOISE_LINE = /^\[[^\]]+\]$/i;

/** 话语/语义群起始（下一行以此开头时，可在上一缓冲结束） */
const DISCOURSE_START: Record<"en" | "es" | "fr", RegExp> = {
...
const INTERNAL_BREAK: Record<"en" | "es" | "fr", RegExp> = {
...
function charCountJa(text: string): number {
...
function endsSentence(text: string, lang: SupportedLanguage): boolean {
  const t = text.trim();
  if (!t) return false;
  if (lang === "ja") return /[。！？!?…]$/.test(t);
  return /[.!?…]$/.test(t);
}

function startsDiscourse(text: string, lang: SupportedLanguage): boolean {
  const t = normalizeText(text);
  if (!t || NOISE_LINE.test(t)) return false;
  if (lang === "ja") {
    return /^(じゃあ|じゃ|それで|でも|だけど|ところで|さて|はい|ええ|あの|えっと)/.test(
      t
    );
  }
  const re = DISCOURSE_START[lang as "en" | "es" | "fr"];
  return re ? re.test(t) : false;
}

function unitCount(text: string, lang: SupportedLanguage): number {
  return lang === "ja" ? charCountJa(text) : wordCount(text);
}

function targetRange(lang: SupportedLanguage): { min: number; max: number } {
  if (lang === "ja") return { min: 18, max: 42 };
  if (lang === "en") return { min: 10, max: 22 };
  return { min: 8, max: 18 }; // es / fr：口语语义群略短
}

function endsIncomplete(text: string, lang: SupportedLanguage): boolean {
  const t = normalizeText(text);
  if (!t || endsSentence(t, lang)) return false;
  if (lang === "ja") {
  ...
  if (lang === "fr" && last === "c'est") return true;
  return dangling[lang]?.has(last) ?? false;
}

function joinTexts(a: string, b: string, lang: SupportedLanguage): string {
  const left = normalizeText(a);
  const right = normalizeText(b);
  if (!left) return right;
  if (!right) return left;
  if (lang === "ja") return `${left}${right}`;
  return `${left} ${right}`;
}
...
function splitOversizedLine(
  ...
  const re = INTERNAL_BREAK[lang as "en" | "es" | "fr"];
  if (!re) return [line];
 ...
 */
export function coalesceIncompleteSemanticGroups(
  ...
  const { min, max } = targetRange(lang);
  const coalesced: TranscriptLine[] = [];
  for (const line of lines) {
    ...
    if (force) {
      const rest = absorbUntilComplete(
        prev!,
        text,
        line.end,
 ...
 */
function stitchTightContinuations(
  lines: TranscriptLine[],
  lang: SupportedLanguage
): TranscriptLine[] {
  if (lang === "ja" || lines.length <= 1) return lines;
  const { max } = targetRange(lang);
  const out: TranscriptLine[] = [];
  for (const line of lines) {
    const text = normalizeText(line.text ?? "");
...
export function shouldMergeTranscriptSentences(
  lang: SupportedLanguage
): boolean {
...
export function looksLikeFragmentedTranscript(
  ...
  if (lang === "ja") {
    const avgChars =
      sample.reduce((n, l) => n + charCountJa(l.text ?? ""), 0) / sample.length;
    const shortRatio =
      sample.filter((l) => charCountJa(l.text ?? "") <= 8).length /
      sample.length;
    const punctRatio =
  ...
  const shortRatio =
    sample.filter((l) => wordCount(l.text ?? "") <= 6).length / sample.length;
  const punctRatio =
    sample.filter((l) => /[.!?…]$/.test((l.text ?? "").trim())).length /
    sample.length;

  if (punctRatio < 0.2 && avgWords < 9) return true;
  if (avgWords < 8 && shortRatio > 0.35) return true;
  if (avgWords < 7 && punctRatio < 0.35) return true;
  return false;
}
